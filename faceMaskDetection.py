    ...
    for j in range(w_out):
      for i_kernel in range(h_k):
        for j_kernel in range(w_k):
          output_conv[i, j] += input[i_kernel+i*stride,j_kernel+j*stride]*kernel[i_kernel, j_kernel]
...
def Convolution_forward(input, kernel,filters, use_batchnorm = True, bias = [[[]]], stride = 1, padding = "valid"):
  ...
  for i in range(filters):
    ...
    for j in range(input_depth):
      temp += correlate2d(input[0,:,:,j], kernel[:,:,j, i], stride = stride, padding = padding)
    output[:,:,i] += temp
...
def DarknetConv(x, filters, size, strides=1, batch_norm=True):
    if strides == 1:
        ...
        print(x.shape)
        x = np.pad(x,((0,0),(1,0),(1,0),(0,0)),'constant') # top left half-padding
        padding = 'valid'
...
# Load weight------------------------------------------------------------
    ...
    kernel = np.fromfile(weight_file, dtype=np.float32, count=np.product(conv_shape))
...
#------------------------------------------------------------------------
    ...
    x = Convolution_forward(input = x, kernel = kernel, filters = filters, bias = bias, stride = strides, padding = padding, use_batchnorm = batch_norm)
    if batch_norm:
        x = BatchNormalization_forward(x, gamma,beta,moving_mean,moving_variance)
...
def combined_non_max_suppression(boxes, scores, iou_threshold, score_threshold):
    ...
    global size
    areas = (x2 - x1 + 1/size) * (y2 - y1 + 1/size) # We add 1, because the pixel at the start as well as at the end counts
    ...
    for batch in range(np.shape(boxes)[0]):
      ...
      for i,box in enumerate(boxes[batch]):
          ...
          #   continue
      for i,box in enumerate(boxes[batch]):
          classes[batch,i] = np.where(scores[batch,i] == scores[batch,i].max())[0][0]          
...
def YoloV3(inputs, size=None, channels=3, anchors=yolo_anchors,
    ...
    outputs = (lambda x: yolo_nms(x, anchors, masks, classes))((boxes_0[:3], boxes_1[:3], boxes_2[:3]))
    end3 = time.time()
...
def Main():    
   ...
   for i in range(len(boxes)):
      print('\t{}, {}, {}'.format(class_names_local[int(classes[i])],np.array(scores[i]),np.array(boxes[i])))
...
major, minor, revision, seen, _ = np.fromfile(weight_file, dtype=np.float32, count=5)
